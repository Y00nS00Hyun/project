...
POSIX-like ACL / RBAC scopes (preview) The Microsoft Entra security principal behind the query token is compared to the permission metadata of documents returned in search results, excluding any documents that don't match on permissions. Access control list (ACL) permissions apply to Azure Data Lake Storage (ADLS) Gen2 directories and files. Role-based access control (RBAC) scopes apply to ADLS Gen2 content and to Azure blobs.

Built-in support for identity-based access at the document level is in preview, available in REST APIs and preview Azure SDK packages that provide the feature. For evidence of feature support, check the SDK version support details.
Microsoft Purview sensitivity labels (preview) Indexer extracts sensitivity labels defined in Microsoft Purview from supported data sources (Azure Blob Storage, ADLS Gen2, SharePoint in Microsoft 365, OneLake). These labels are stored as metadata and evaluated at query time to enforce user access based on Microsoft Entra tokens and Purview policy assignments. Labels are also surfaced through knowledge sources and the agentic retrieval response, allowing AI agents and chat apps consuming a knowledge base to receive the same label-aware filtering. This approach aligns Azure AI Search authorization with your enterprise's Microsoft Information Protection model.
SharePoint in Microsoft 365 ACLs (preview) Azure AI Search indexers extract permission metadata from supported SharePoint content and use it for query-time access checks. For supported content, principals, group relationships, synchronization behavior, and permissions, see Use a SharePoint indexer to ingest permission metadata.
For indexed knowledge sources, ingestionPermissionOptions can't be combined with assetStore. Therefore, image serving (preview) isn't available when native document-level permission ingestion is enabled.
...
This approach is useful for systems with custom access models or non-Microsoft security frameworks. For more information about this approach, see Security filters for trimming results in Azure AI Search.
...
Azure SDK for Python prerelease package. Check the changelog for the latest preview version that supports ACL and RBAC scope ingestion.
Azure SDK for .NET prerelease package. Check the changelog for the latest preview version that supports ACL and RBAC scope ingestion.
Azure SDK for Java prerelease package. Check the changelog for the latest preview version that supports ACL and RBAC scope ingestion.
For the push model approach:

Confirm that your index schema is created with a preview or prerelease SDK and that the schema has permission filters.
...
Use Indexers - Create (REST API), Knowledge Sources - Create (REST API), or an equivalent Azure SDK API to create the indexer, index, and data source.
If your skillset chunks documents, such as with the Text Split skill for integrated vectorization, the permission metadata fields move from indexer field mappings to index projections. See Choose where to populate ACL fields.
...
For permission requirements, supported group relationships, permission synchronization, and limitations, see Use a SharePoint indexer to ingest permission metadata.

If your skillset chunks documents (for example, with the Text Split skill for integrated vectorization), the ACL fields move from indexer field mappings to index projections. See Choose where to populate ACL fields.
...
Configure your index, data source, and indexer (for scheduling purposes) by using the latest preview REST API or a preview SDK that supports Purview label ingestion.
Enable a system-assigned managed identity on your search service. User-assigned managed identities aren't supported for Purview label extraction - the service's own identity must hold the elevated Purview permissions. Then have your tenant global administrator or privileged role administrator grant the required access to allow the search service to authenticate with Microsoft Purview and extract label metadata.
...
Direct query API (/docs/search): Attach the user's Microsoft Entra token in x-ms-query-source-authorization. Administrators can also issue elevated read requests for auditable investigations. For setup and examples, see Query-time enforcement of Microsoft Purview sensitivity labels.

Knowledge sources and agentic retrieval (MCP): Set ingestionPermissionOptions to include sensitivityLabel on the knowledge source. The retrieve action and MCP knowledge_base_retrieve tool return per-reference sensitivityLabelInfo and response-level metadata.responseSensitivityLabelInfo that clients can use for display banners and policy enforcement. For setup, see Creating knowledge source and Inspect sensitivity label metadata in retrieve responses.

If the knowledge source points to a chunked index, such as one populated through integrated vectorization or a custom Text Split skill, the skillset must also project the sensitivity label to each chunk row. Without this projection, chunk-level references aren't filtered.
