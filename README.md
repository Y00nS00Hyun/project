...
**공유폴더가 Source of Truth다.** 기본 구성에서 공유폴더는 `/data/shared:ro`로 마운트되며,
수집·검색·파싱·다운로드 경로는 원본을 수정하지 않는다. DB는 공유폴더의 파생물이고,
둘이 어긋나면 공유폴더가 옳다.

선택 기능인 **원본 파일 관리**를 명시적으로 켠 경우에만, 시스템 관리자가 웹에서 원본 파일의
이름을 바꾸거나 기존 폴더로 옮기고, 새 폴더를 만들 수 있다. 기본값은 비활성이며, 파일·폴더
삭제, 폴더 이름 변경, 업로드, 복사, 본문 편집은 지원하지 않는다(§6, `docs/deployment.md` §8.1).
...
기본은 **비활성**이다. 운영자가 켜면(`docs/deployment.md` §8.1) **시스템 관리자에게만** 문서 상세
화면의 `문서 관리` 메뉴와 검색 화면 폴더 목록의 `+ 새 폴더` 버튼이 보인다. 일반 사용자에게는
보이지 않고, API를 직접 호출해도 거부된다.
...
이름 변경과 이동은 내용 변경이 아니다. document_id, current/latest revision이 그대로이고 새
revision도 생기지 않으며, 권한·즐겨찾기·채팅·인용이 같은 문서에 계속 연결된다.

**새 폴더**는 최상위 또는 기존 폴더 아래에 한 단계만 만든다. 중간 경로는 자동으로 만들지 않고,
같은 이름의 파일이나 폴더가 이미 있으면 거부한다(409).

**폴더 목록은 목적이 다른 두 가지다.**

| API | 용도 | 빈 폴더 |
|---|---|---|
| `/folders` | 모든 사용자의 검색 화면 폴더 목록. 권한을 통과한 current READY 문서가 있는 폴더만 | 표시하지 않음 |
| `/admin/directories` | 시스템 관리자의 파일 관리용 실제 디렉터리 목록. 기능이 켜졌을 때만 | 표시 |

`/admin/directories`는 일반 사용자에게 실제 폴더 이름을 보여주기 위한 API가 아니다. 그래서 새로
만든 빈 폴더는 검색 화면 폴더 목록에는 나타나지 않고, 관리자의 위치·이동 선택 목록에는 바로
나타난다. 그 폴더에 READY 문서가 생기면 그때 검색 화면 폴더 목록에도 표시된다.

경로 이탈과 symlink 경로는 거부하고, 파일 확장자는 바꿀 수 없다. 옮길 위치에 같은 이름의 파일이
있으면 덮어쓰지 않고 거부한다(409). 처리 중인 문서도 거부한다. 관리 행위는 `audit_logs`에
`DOCUMENT_RENAMED` / `DOCUMENT_MOVED` / `FOLDER_CREATED`로 기록된다.
