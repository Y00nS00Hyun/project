...
| Method | Path | 인증 | 비고 |
| --- | --- | --- | --- |
| `GET` | `/api/v1/documents/{document_id}/text` | 필수 | 추출된 원문 미리보기 |

`offset` / `limit`(기본 20, 최대 50) 페이징이다. ACL을 먼저 확인하고 current READY
revision의 본문만 반환한다. 권한이 없으면 상세와 동일하게 404다. 문서 상세 응답에는
본문을 싣지 않는다. 현재 합계 23개.

---

## 32. v1.3 Open Issues

### OI-12. 비밀번호 재설정 — **구현됨 (메일 발송은 없음)**

관리자가 `auth.cli reset-password`로 일회용 토큰을 발급하고, 사용자가 그 토큰으로
`/reset-password` 화면에서 직접 새 비밀번호를 정한다. 관리자는 원문 비밀번호를 보지 않고
임시 비밀번호도 발급하지 않는다. 토큰은 한 번 쓰면 소진되고 30분 뒤 만료된다.

남아 있는 한계는 **전달 경로가 수동**이라는 점이다. 메일 인프라가 없어 토큰을 운영자가
직접 전달해야 한다.
