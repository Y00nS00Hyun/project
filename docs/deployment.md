...
backend
 ├── 공유폴더        read-only 마운트 (수집·검색·파싱·다운로드)
 ├── 공유폴더(쓰기)  파일 관리 기능을 켰을 때만 read-write 마운트 (§8.1)
...
기본 구성에서 시스템은 공유폴더의 **읽기 전용 소비자**다. 수집·검색·파싱·다운로드는 모두
`/data/shared`만 사용하고 원본을 수정하지 않으며, `:ro` 마운트가 그것을 코드가 아니라 커널
수준에서 보장한다. 이 마운트는 파일 관리 기능을 켜도 바뀌지 않는다.

예외는 §8.1의 선택 기능 하나다. 명시적으로 켠 경우에만 시스템 관리자가 원본 파일의 이름을
바꾸거나 기존 폴더로 옮기고, 새 폴더를 만들 수 있다. 파일·폴더 삭제, 폴더 이름 변경, 업로드,
복사, 본문 편집은 없다.
...
```bash
docker compose config | grep -A2 '/data/shared'      # /data/shared 는 read_only: true 여야 한다
```

---

## 8.1 원본 파일 관리 (선택, 기본 비활성)

시스템 관리자가 웹에서 원본 파일의 **이름 변경**, **기존 폴더로 이동**, **새 폴더 생성**을 할 수
있게 한다. **기본은 비활성**이며, 기본 `compose.yaml`만으로는 켜지지 않는다.

| 기능 | 상태 |
|---|---|
| 파일명 변경 · 기존 폴더로 이동 · 이름 변경 + 이동 · 새 폴더 생성 | 지원 |
| 파일 삭제 · 새 파일 업로드 · 폴더 삭제 · 폴더 이름 변경 · 파일 복사 · 문서 본문 편집 | 미지원 |

### 구조

```text
/data/shared         read-only    수집·검색·파싱·다운로드         (SHARED_ROOT)
/data/shared-write   read-write   이름 변경·이동·폴더 생성 전용   (SHARED_WRITE_ROOT)
```

둘은 **같은 호스트 공유폴더**(`SHARED_FOLDER_HOST_PATH`)를 가리킨다. 수집·검색 코드는 계속
`/data/shared`만 사용한다. 쓰기 마운트는 `compose.file-management.yaml`에만 있으며, 이 파일이
`DOCUMENT_FILE_MANAGEMENT_ENABLED=true`, `SHARED_WRITE_ROOT=/data/shared-write`, read-write
마운트, `SHARED_FOLDER_GID` 그룹 추가를 함께 넣는다. flag만 켜고 쓰기 마운트가 없으면 기능은
꺼진 것으로 취급된다.
...
| API | 용도 | 빈 폴더 | 사용 가능 |
|---|---|---|---|
| `/folders` | 검색 화면 폴더 탐색. 권한을 통과한 current READY 문서가 있는 폴더만 | 표시 안 함 | 모든 로그인 사용자 |
| `/admin/directories` | 파일 관리용 실제 디렉터리 목록(쓰기 마운트 기준). 새 폴더 위치·이동 대상 선택에만 사용 | 표시 | 시스템 관리자, 기능이 켜졌을 때만 |

`/admin/directories`는 일반 사용자에게 실제 폴더 이름을 노출하기 위한 API가 아니다. 목록은
상대 경로만 담고, symlink는 포함하지도 따라가지도 않는다.

### 활성화

`.env`에 추가하고 backend를 다시 만든다.

```bash
COMPOSE_FILE=compose.yaml:compose.file-management.yaml
SHARED_FOLDER_GID=<공유폴더에 쓰기 권한이 있는 그룹 id>
```

```bash
docker compose up -d backend
docker compose exec backend printenv DOCUMENT_FILE_MANAGEMENT_ENABLED SHARED_WRITE_ROOT
```

끄려면 두 줄을 지우고 `docker compose up -d backend`를 다시 실행한다. 쓰기 마운트와 flag가 함께
사라진다.

### 공유폴더 권한 (운영 주의)

backend 컨테이너는 **uid 10001**로 실행된다. 파일 이름을 바꾸거나 옮기려면 파일이 **들어 있는
디렉터리와 옮겨 갈 디렉터리** 모두에, 새 폴더를 만들려면 **상위 디렉터리**에 쓰기 권한이 필요하다.
그래서 해당 디렉터리가 `SHARED_FOLDER_GID` 그룹에 쓰기 가능해야 한다. 권한이 없으면 요청은
"공유폴더에 쓰기 권한이 없습니다."(503)로 거부되고, 파일과 폴더는 그대로 남는다.

현재 VM은 최상위 공유폴더가 `755`, 하위 폴더가 `775`다. 따라서:

- 하위 `775` 폴더 안이나 그 사이의 이름 변경·이동·새 폴더 생성은 동작한다
- **최상위에 새 폴더 만들기는 503으로 실패할 수 있다**
- **최상위로 파일을 옮기거나, 최상위에 있는 파일의 이름을 바꾸거나 옮기는 작업도 503으로 실패할
  수 있다**

이는 코드 버그가 아니라 **파일시스템 권한 문제**다. 시스템은 기존 디렉터리의 권한을 바꾸지
않으며, 권한을 어떻게 바꿀지는 공유폴더 운영 정책에 따른 결정이라 이 절차에서도 변경하지 않는다.
(웹에서 새로 만든 폴더만 상위 폴더의 그룹과 권한 비트를 이어받는다. 그래야 파일서버를 쓰는
사람도 그 폴더에 문서를 넣을 수 있다.)

### 의미와 보호 규칙

- 내용 변경이 아니므로 document_id, current/latest revision, 권한·즐겨찾기·채팅·인용이 그대로
  유지되고 새 revision은 생기지 않는다
- 과거 revision의 `source_path_at_ingest`는 수집 당시의 위치 기록(provenance)이라 바뀌지 않는다
- 다음 수집 scan은 옮겨진 파일을 이미 알고 있는 경로로 보고, 새 문서나 revision을 만들지 않는다
- 시스템 관리자만 사용할 수 있다. 일반 사용자는 403, 기능이 꺼져 있으면 503이다
- 경로 이탈(`..`, 절대경로)과 symlink인 대상 폴더는 거부한다(422)
- 파일 확장자는 바꿀 수 없다(422)
- 옮길 위치에 같은 이름의 파일이 있으면 덮어쓰지 않고 거부한다(409)
- 새 버전을 수집·처리 중인 문서는 거부한다(409). 처리가 끝난 뒤 다시 시도한다
- 변경은 `audit_logs`에 `DOCUMENT_RENAMED` / `DOCUMENT_MOVED`로 기록된다. 대상은 문서이고,
  이전·이후 상대 경로가 함께 남는다

### 새 폴더 생성

- 최상위 또는 기존 폴더 아래에 **한 단계만** 만든다. 존재하지 않는 중간 경로는 만들지 않는다(422)
- 같은 이름의 파일이나 폴더가 이미 있으면 합치거나 덮어쓰지 않고 거부한다(409)
- 폴더 이름은 경로 한 단계만 허용한다. `..`, 절대경로, 경로 구분자, 제어 문자는 거부한다(422)
- symlink를 거치는 상위 경로는 거부한다(422)
- 새로 만든 빈 폴더는 **검색 화면 폴더 목록(`/folders`)에는 나타나지 않고**, 관리자의
  위치·이동 선택 목록(`/admin/directories`)에는 바로 나타난다. 그 폴더에 READY 문서가 생기면
  `/folders`에도 표시된다
- 생성은 `audit_logs`에 `FOLDER_CREATED`로 기록된다. 대상은 폴더이고 상대 경로가 남는다
...
파일 관리 권한     최상위 공유폴더가 755인 VM에서는 최상위 대상 이름 변경·이동·폴더 생성 불가 (§8.1)
