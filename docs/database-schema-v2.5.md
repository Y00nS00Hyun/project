...
CREATE TABLE document_revisions (
    ...
    /*
     * document_date  (migration 0007)
     *
     * 문서 표지/앞부분에 '명시된' 작성일. 예: 표지 "2025. 11. 26." -> 2025-11-26
     *
     * document_year 와 독립적으로 판정한다. 표지에 "2025.01.05" 와
     * "2025.12.20" 이 함께 있으면 연도는 2025 로 확정되지만 날짜는 NULL 이다.
     * 반대 방향은 없다 -- 날짜가 있으면 연도도 정해진다.
     *
     * 연도만 알 수 있을 때 1월 1일 같은 날짜를 만들어내지 않는다. 문서가
     * 말하지 않은 사실을 저장하는 것이기 때문이다.
     *
     * DATE 이고 TIMESTAMPTZ 가 아니다 -- 표지는 '날'을 적지 시각과 timezone 을
     * 적지 않는다. 시각으로 저장하면 둘 다 지어내게 되고, 지어낸 timezone 이
     * 독자에 따라 표시되는 날짜를 하루 움직인다.
     *
     * documents 가 아니라 document_revisions 에 둔다. 표지 날짜는 revision 의
     * 속성이다 -- 보고서를 1월에 재발행하면 새 표지 날짜가 생기지만 과거
     * revision 은 자기가 적은 날짜를 그대로 유지해야 한다.
     *
     * 인덱스를 두지 않는다. 이 컬럼으로 필터하거나 정렬하지 않으며, 문서 상세
     * 화면에서 PK 로 찾은 한 건을 표시할 뿐이다.
     */
    document_date DATE,
